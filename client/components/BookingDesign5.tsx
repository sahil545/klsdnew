...
export default function BookingDesign5({ 
  onReserveClick 
}: { 
  onReserveClick: () => void 
}) {
  ...
  return (
    <div className="w-full max-w-sm">
      {/* Modern Card with Ray Pattern Header */}
      <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
        
        {/* Top Section with Logo Ray Pattern */}
        <div className="relative bg-gradient-to-br from-blue-500/80 to-blue-600/80 p-6 text-white overflow-hidden">
          
          {/* Subtle Ray Pattern */}
          <div className="absolute inset-0 opacity-20">
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
              {/* Center point for rays */}
              <div className="relative w-32 h-32">
                {/* Multiple rays emanating from center */}
                {[...Array(8)].map((_, i) => (
                  <div
                    key={i}
                    className="absolute w-0.5 h-16 bg-white/30 origin-bottom"
                    style={{
                ...
                {[...Array(8)].map((_, i) => (
                  <div
                    key={i}
                    className="absolute w-0.5 h-12 bg-white/20 origin-bottom"
                    style={{
