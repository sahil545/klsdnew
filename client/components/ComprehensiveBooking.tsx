...
export default function ComprehensiveBooking() {
  ...
  const productId = 34450;
  const taxRate = 0.07; // 7% tax
  const selectedPkg = packages[selectedPackage];
  const subtotal = selectedPkg.price * guestCount;
  ...
  const loadProductData = async () => {
    try {
      ...
      const mockProduct = {
        ...
        regular_price: 89,
        sale_price: null,
        description: "Experience the iconic Christ of the Abyss statue in crystal-clear waters.",
        short_description: "Visit the famous 9-foot bronze Christ statue underwater",
        stock_quantity: 25,
        in_stock: true,
        images: [
          {
      ...
      const mockAvailability = {
        available_dates: [
          { date: "2024-01-20", times: ["8:00 AM", "12:00 PM", "3:00 PM"], spots_available: 25 },
          { date: "2024-01-21", times: ["8:00 AM", "12:00 PM", "3:00 PM"], spots_available: 20 },
          { date: "2024-01-22", times: ["8:00 AM", "12:00 PM", "3:00 PM"], spots_available: 18 }
        ]
      };
...
You can view this order in your WooCommerce admin under Orders.`);
  ...
  const incrementGuests = () => {
    if (guestCount < 25) setGuestCount(guestCount + 1);
  };
  ...
  const selectDate = (date: string) => {
    ...
    setSelectedTime('8:00 AM'); // Default time
  };
  ...
  return (
    <section className="py-24 bg-gradient-to-br from-ocean/5 via-ocean-light/10 to-ocean/5">
      <div className="container mx-auto px-4">
        ...
        {/* Connection Status */}
        <div className="mb-8 p-4 rounded-lg border bg-white/50">
          <div className="flex items-center gap-2 mb-2">
            <div className={`w-3 h-3 rounded-full ${connectionTest === true ? 'bg-green-500' : connectionTest === false ? 'bg-red-500' : 'bg-yellow-500'}`}></div>
            <span className="font-semibold">WooCommerce Connection Status:</span>
            <span className={connectionTest === true ? 'text-green-600' : connectionTest === false ? 'text-red-600' : 'text-yellow-600'}>
              {connectionTest === true ? 'Connected ✅' : connectionTest === false ? 'Failed ❌' : 'Testing...'}
            </span>
          </div>
          {productData && (
            <div className="text-sm text-gray-600">
              <p><strong>Live Product:</strong> {productData.name}</p>
              <p><strong>WooCommerce Price:</strong> ${productData.price}</p>
              <p><strong>Stock:</strong> {productData.stock_quantity} available</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 mb-12">
          <h3 className="text-2xl font-bold text-center mb-8">Select Your Tour Details</h3>
          
          <div className="grid md:grid-cols-2 gap-8">
            {/* Date Selection */}
            <div>
              <label className="block text-lg font-semibold mb-4">Choose Your Date</label>
              <div className="grid gap-3">
                {availability?.available_dates?.map((dateOption: any, index: number) => (
                  <button
                    key={index}
                    onClick={() => selectDate(dateOption.date)}
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <div className="text-sm text-gray-600">
                          Times: {dateOption.times.join(', ')}
                        </div>
                      </div>
                      <div className="text-sm font-medium text-green-600">
                        {dateOption.spots_available} spots left
                      </div>
                    </div>
                  </button>
                ))}
            ...
            {/* Guest Count & Pricing */}
            <div>
              <label className="block text-lg font-semibold mb-4">Number of Guests</label>
              ...
              <div className="bg-gray-50 rounded-lg p-6 mb-6">
                ...
                <div className="border-t pt-4 space-y-2">
                  <div className="flex justify-between">
                    <span>Subtotal ({guestCount} guests @ ${selectedPkg.price})</span>
              ...
              {/* Book Now Button */}
              <Button 
                className="w-full bg-orange-600 hover:bg-orange-700 text-white text-lg font-semibold py-4"
                onClick={() => handleBookNow(selectedPackage)}
                disabled={!selectedDate || !connectionTest}
              >
                {!selectedDate ? 'Select Date First' : `Book Now - $${total.toFixed(2)}`}
              </Button>
            </div>
          </div>
        </div>
              ...
              <CardHeader className={`text-center ${pkg.popular ? 'pt-12' : 'pt-8'}`}>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-center gap-2">
                    <span className="text-4xl font-bold text-ocean">${pkg.price}</span>
        ...
        <div className="grid md:grid-cols-4 gap-6 mb-12">
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground">High-quality underwater photography included</p>
          </div>
          <div className="text-center">
            <div className="w-16 h-16 bg-ocean/10 rounded-2xl flex items-center justify-center mx-auto mb-4">
            ...
            <p className="text-sm text-muted-foreground">PADI certified guides & inspected equipment</p>
          </div>
          <div className="text-center">
            <div className="w-16 h-16 bg-ocean/10 rounded-2xl flex items-center justify-center mx-auto mb-4">
              <Waves className="w-8 h-8 text-ocean" />
            </div>
            <h3 className="font-semibold text-foreground mb-2">Perfect Conditions</h3>
            <p className="text-sm text-muted-foreground">Crystal clear waters 60-100ft visibility</p>
          </div>
          <div className="text-center">
            <div className="w-16 h-16 bg-ocean/10 rounded-2xl flex items-center justify-center mx-auto mb-4">
            ...
            <p className="text-sm text-muted-foreground">Encounter tropical fish & coral reefs</p>
          </div>
        </div>
        ...
        <div className="text-center">
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-8 border border-ocean/10 max-w-2xl mx-auto">
            <div className="flex items-center justify-center gap-2 mb-4">
              ...
              <span className="font-semibold text-foreground">Still not sure?</span>
            </div>
            <p className="text-muted-foreground mb-6">
              Join thousands of satisfied adventurers. Call us at
              <span className="text-ocean font-semibold"> (305) 391-4040</span> for personalized assistance.
            </p>
